...
// RetryAttempts sets whether failing S3 S3Requests may be retried to cope
...
func (req *S3Request) url() (*url.URL, error) {
...
// Query prepares and runs the req S3Request.
// If resp is not nil, the XML data contained in the response
// body will be unmarshalled on it.
func (s3 *S3) Query(req *S3Request, resp interface{}) error {
	err := s3.prepare(req)
	if err != nil {
		return err
	}
...
func (s3 *S3) prepare(req *S3Request) error {
	if !req.prepared {
		req.prepared = true
		if req.method == "" {
			req.method = "GET"
...
func (s3 *S3) run(req *S3Request) (*http.Response, error) {
	if debug {
		log.Printf("Running S3 S3Request: %#v", req)
	}
